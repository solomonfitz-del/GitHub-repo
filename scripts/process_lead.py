...
def deploy_demo(lead):
    ...
    try:
        # Call builder/generator.py
        subprocess.run([
            "python3", os.path.join(REPO_ROOT, "builder/generator.py"),
            "--name", business_name,
        ...
        deploy_script = os.path.join(REPO_ROOT, "scripts/deploy_site.sh")
...
def send_outreach(lead):
    ...
    try:
        ...
        if not os.path.exists(template_path):
            # Fallback if I messed up the path
            template_path = os.path.join(REPO_ROOT, "pipeline/templates/outreach_email.txt")
